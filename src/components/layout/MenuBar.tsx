...
export function MenuBar() {
  ...
  return (
    <div className="sticky top-0 z-50">
      {/* Menu Bar */}
      <div
        className="h-5 flex items-center border-b border-black"
        style={{
      ...
      >
        {/* Apple Menu Icon — doubles as mobile menu toggle */}
        <button
          onClick={() => setMenuOpen((o) => !o)}
          className="px-2 h-full flex items-center hover:bg-black hover:text-white cursor-pointer border-0 bg-transparent"
          aria-label="Menu"
        >
          <svg
            width="14"
        ...
        >
          <span className="hidden sm:inline">MacOSInspiration</span>
        ...
        <Link
          href="/"
          className="hidden sm:flex px-3 h-full items-center hover:bg-black hover:text-white"
        >
          Gallery
        </Link>
        ...
        <div className="hidden sm:flex px-3 h-full items-center text-[10px] text-gray-600">
      ...
      {menuOpen && (
        <div
          className="sm:hidden border-b border-black"
          style={{
            background: 'linear-gradient(180deg, #EEEEEE 0%, #CCCCCC 100%)',
            fontFamily: "'ChicagoFLF', 'Geneva', 'Verdana', sans-serif",
            fontSize: '12px',
        ...
        >
          <Link
            ...
            className="block px-4 py-1.5 border-b border-[#AAAAAA] hover:bg-black hover:text-white"
          >
            Gallery
          </Link>
          <Link
          ...
          <div className="px-4 py-1.5 text-[10px] text-gray-600">
            Curated by Stoodio.com
          </div>
        </div>
      )}
